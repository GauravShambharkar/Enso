  ...
  return (
    <>
      ...
      <aside
        ...
        className="hidden md:flex fixed top-0 left-0 h-screen flex-col transition-[width] duration-150 ease-out z-40"
      >
        {/* Logo row */}
        <div
        ...
        >
          ...
          <button
            ...
            className="cursor-pointer transition-colors duration-150 bg-transparent border-none p-0"
      ...
      <div
        ...
        className="flex md:hidden fixed bottom-0 left-0 right-0 h-14 items-center justify-around px-3 z-50 shadow-md"
      >
        {NAV.map(({ label, href, icon: Icon }) => {
          const active = path.startsWith(href);
          return (
            <Link
              key={href}
              href={href}
              className="flex flex-col items-center justify-center flex-1 py-1 gap-1 text-[10px] no-underline transition-colors"
              style={{
